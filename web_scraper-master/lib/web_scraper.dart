...
class WebScraper {
  ...
  //커스텀하여 엘리먼트 정보들을 가져오는 메서드
  List<Map<String, dynamic>> customGetElement(
      String address, List<String> attribs,
      {String? extraAddress}) {
    // Attribs are the list of attributes required to extract from the html tag(s) ex. ['href', 'title'].
    if (_document == null) {
      throw WebScraperException(
    ...
    for (int i = 0; i < elements.length; i++) {
      ...
      //href 스크랩
      elementData[i]['href'] = elements[i].firstChild?.attributes['href'] ?? '';

      //이미지url, 타이틀 스크랩
      if (elements[i].getElementsByTagName('img').length != 0) {
        elementData[i]['img'] =
            elements[i].getElementsByTagName('img')[0].attributes['src'];
        elementData[i]['title'] = utf8
            .decode(
                (elements[i].getElementsByTagName('img')[0].attributes['alt'] ??
                        '')
                    .runes
                    .toList())
            .toString();
        ...
        elementData[i]['img'] = '';
        elementData[i]['title'] = utf8
            .decode((elements[i].getElementsByClassName('headline')[0].text)
                .runes
                .toList())
            .toString();
      }
      ...
      //텍스트 스크랩
      if (elements[i].getElementsByClassName('text').length != 0) {
        elementData[i]['text'] = utf8
            .decode(elements[i]
      ...
      //소제목 스크랩
      if (elements[i].getElementsByClassName('list_disc_item').length != 0) {
        var subTitle = <String, String>{};
        var subElements = elements[i].getElementsByClassName('list_disc_item');
        subElements.forEach((element) {
          final href = element.parent?.attributes['href'];
          if (href != null) {
            subTitle[href] =
                utf8.decode(element.text.runes.toList()).toString();
          }
        });
        elementData[i]['subTitle'] = subTitle;
      }
  ...
  //네이버뉴스 기사 스크랩
  List<Map<String, dynamic>> customGetElementNaver(
      String address, List<String> attribs,
      {String? extraAddress}) {
    // Attribs are the list of attributes required to extract from the html tag(s) ex. ['href', 'title'].
    if (_document == null) {
    ...
    //print(elements[0].getElementsByClassName('photo')[0].innerHtml);
    elements.forEach((element) {
      final newsElement = <String, dynamic>{};
      final photoElement = element.getElementsByClassName('photo');
      try {
        if (photoElement.length != 0) {
          //사진있는 뉴스일 때
          newsElement['href'] = photoElement[0].children[0].attributes['href'];
          newsElement['img'] =
              photoElement[0].children[0].children[0].attributes['src'];
          newsElement['title'] = cp949
              .decodeString(
                  photoElement[0].children[0].children[0].attributes['alt'])
              .trim();
        } else {
          //사진없는 뉴스일 때
          newsElement['href'] = element
              .getElementsByTagName('dt')[0]
              .children[0]
              .attributes['href'];
          newsElement['title'] = cp949
              .decodeString(
                  element.getElementsByTagName('dt')[0].children[0].text)
              .trim();
        }
        //기사텍스트
        newsElement['text'] = cp949.decodeString(
            element.getElementsByTagName('dd')[0].children[0].text);

        elementData.add(newsElement);
      } on ArgumentError catch (e) {
        print(
            '기사 인코딩 에러(euc-kr)(cp949)(null-value)${element.getElementsByTagName('a')[0].attributes['href']}');
      } catch (e) {
        print(e);
        throw e;
      }
